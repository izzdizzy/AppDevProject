...
class Card:
    ...
    def __init__(self, first_name, last_name, card_number, expiry_date, cvc_number, card_id=None):
...
class Review:
    ...
    def __init__(self, ebook_id, stars, message, reviewer_name, review_id=None):
        # If review_id is not provided, generate a new one
        if review_id is None:
            Review.count_id += 1
            self.__review_id = Review.count_id
        else:
            self.__review_id = review_id
            # Update count_id if the provided review_id is greater
            if review_id > Review.count_id:
                Review.count_id = review_id

        self.__ebook_id = ebook_id  # ID of the book being reviewed
        self.__stars = stars  # Star rating (1-5)
        self.__message = message  # Review message
        self.__reviewer_name = reviewer_name  # Name of the reviewer
